...
var SetBody = {
    ...
    setPolygon: function (radius, sides, options) {
        return this.setBody(
            { type: "polygon", sides: sides, radius: radius },
            options
        );
    },
    ...
    setTrapezoid: function (width, height, slope, options) {
        return this.setBody(
            { type: "trapezoid", width: width, height: height, slope: slope },
            options
        );
    },
    ...
    setExistingBody: function (body, addToWorld) {
        ...
        if (this._originComponent) {
            ...
            if (FuzzyEquals(comx, 0.5) && FuzzyEquals(comy, 0.5)) {
                ...
                this.setOrigin(
                    rx + cx / this.displayWidth,
                    ry + cy / this.displayHeight
                );
            }
        }
    ...
    setBody: function (config, options) {
        ...
        switch (shapeType) {
            ...
            case "circle":
                var radius = GetFastValue(
                    config,
                    "radius",
                    Math.max(bodyWidth, bodyHeight) / 2
                );
            ...
            case "fromVerts":
                ...
                if (verts) {
                    ...
                    if (this.body && !this.body.hasOwnProperty("temp")) {
                        ...
                        var flagInternal = GetFastValue(
                            config,
                            "flagInternal",
                            false
                        );
                        var removeCollinear = GetFastValue(
                            config,
                            "removeCollinear",
                            0.01
                        );
                        var minimumArea = GetFastValue(
                            config,
                            "minimumArea",
                            10
                        );
            ...
            case "fromPhysicsEditor":
                body = PhysicsEditorParser.parseBody(
                    bodyX,
                    bodyY,
                    config,
                    options
            ...
            case "fromPhysicsTracer":
                body = PhysicsJSONParser.parseBody(
                    bodyX,
                    bodyY,
                    config,
                    options
